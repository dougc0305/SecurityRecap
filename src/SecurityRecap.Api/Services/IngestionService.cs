...
public class IngestionService : IIngestionService
{
    ...
    public async Task<Guid> IngestReportAsync(Guid tenantId, Guid propertyId, Stream pdfStream, string fileName)
    {
        // Verify property belongs to tenant
        var property = await _db.Properties
            .FirstOrDefaultAsync(p => p.Id == propertyId && p.TenantId == tenantId)
        ...
        foreach (var veh in result.Vehicles)
        {
            ...
            if (!string.IsNullOrWhiteSpace(veh.ViolationType))
            {
                ...
                {
                    Id = Guid.NewGuid(),
                    IncidentId = report.Incidents.FirstOrDefault()?.Id
                        ?? _db.Incidents.Local.FirstOrDefault(i => i.ReportId == report.Id)?.Id
                        ?? Guid.Empty,
        ...
        _logger.LogInformation(
            "Ingested report {ReportId}: {IncidentCount} incidents, {VehicleCount} vehicles",
            report.Id, result.Incidents.Count, result.Vehicles.Count);
